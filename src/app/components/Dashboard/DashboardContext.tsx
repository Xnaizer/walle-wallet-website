...
type DashboardAction =
  | {
      type: "CONNECT_WALLET";
      payload: { address: string; hasPin: boolean; isFirstTime: boolean };
    }
  ...
  | {
      type: "UPDATE_CARD";
      payload: { cardId: string; updates: Partial<WalletCard> };
    }
  ...
  | {
      ...
      payload: { name: string; profileImage: string | null };
    }
...
const dashboardReducer = (
  ...
  switch (action.type) {
    case "CONNECT_WALLET":
      const userName = `${action.payload.address.slice(
        0,
        6
      )}...${action.payload.address.slice(-4)}`;
      return {
        ...state,
        ...
        //   (!action.payload.isFirstTime && action.payload.hasPin),
    ...
    case "DELETE_CARD":
      return {
        ...
        selectedCardIndex:
          state.selectedCardIndex > 0 ? state.selectedCardIndex - 1 : 0,
      };
